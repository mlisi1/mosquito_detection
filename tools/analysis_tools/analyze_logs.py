...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if metric not in log_dict[epochs[-2]].keys():
                if 'mAP' in metric:
                    raise KeyError(
            ...
            if 'mAP' in metric:
                ...
                if args.start_epoch is not None:
                    ...
                    xs = xs[args.start_epoch:-1]
                    ys = ys[args.start_epoch:-1]

                if args.eval_interval is not None:
                    
                    xs = xs[0:args.eval_interval]
                    ys = ys[0:args.eval_interval]
    ...
    if args.out is None:
        ...
        out_path = os.path.abspath(os.path.join(os.path.dirname(args.json_logs[0]), '..', '..', args.out))

        print(f'save curve to: {out_path}')
